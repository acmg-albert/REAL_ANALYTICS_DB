"""
Zillow Median Sale Price Scraper

This module handles the downloading of median sale price data from Zillow.
...
class MedianSalePriceScraper:
    """负责从Zillow下载房屋销售中位数价格数据的类"""
    
    CSV_URL = "https://files.zillowstatic.com/research/public_csvs/median_sale_price/Metro_median_sale_price_uc_sfrcondo_month.csv"
    ...
    def _validate_data(self, df: pd.DataFrame) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            # 检查日期列数量（从2008年2月到现在）
            expected_cols = (datetime.now().year - 2008) * 12 + datetime.now().month + 10  # 加10是因为从2008年2月开始
            if len(date_cols) < 100:  # 根据需求文档要求至少100列
                return False, f"日期列数量不足，当前有 {len(date_cols)} 列，预期至少100列"
            
            # 验证日期列格式（YYYY-MM-DD格式）
            ...
            # 检查最早的日期是否是2008年2月
            earliest_date = min(date_cols, key=lambda x: datetime.strptime(x, '%Y-%m-%d'))
            if datetime.strptime(earliest_date, '%Y-%m-%d').strftime('%Y-%m') != '2008-02':
                return False, f"最早的日期不是2008年2月: {earliest_date}"
    ...
    def scrape(self) -> Path:
        ...
        try:
            ...
            output_path = Path("data") / f"zillow_median_sale_price_{timestamp}.csv"
