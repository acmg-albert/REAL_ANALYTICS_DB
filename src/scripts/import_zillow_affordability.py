"""Script to import Zillow affordability data to Supabase."""
    ...
    with tqdm(total=total_rows, desc="Importing data") as pbar:
        for start_idx in range(0, total_rows, batch_size):
            try:
                ...
                result = supabase.client.table('zillow_new_homeowner_affordability_down_20pct')\
                    .upsert(records, on_conflict='region_id,date')\
                    .execute()
                
                # Update progress
                rows_imported = len(result.data) if result.data else 0
